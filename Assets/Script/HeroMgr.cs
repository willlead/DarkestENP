...
public class HeroMgr : MonoBehaviour {
    ...
    private float mouseSesitivity = 2.0f;
    ...
    void Awake()
    {
        ...
        m_Walk = m_Animation.GetClip("Run");
        m_Walk = m_Animation.GetClip("Jump");
        m_Walk = m_Animation.GetClip("Fall");

        m_Attack = m_Animation.GetClip("Attack");
	...
	void Update () {
        ...
        //마우스 좌우 상하 바라보는 시점 변경
        {
            rotLeftRight = Input.GetAxis("Mouse X") * mouseSesitivity;
            transform.Rotate(0f, rotLeftRight, 0f);

            verticalRotation -= Input.GetAxis("Mouse Y") * mouseSesitivity;
            // 현재값이 최대값보다 높으면 최대값, 최소값보다 작으면 최소값 출력
        ...
        Vector3 vMovemont = (m_vMoveDirection) + new Vector3(0, m_fVerticalSpeed, 0);
        vMovemont *= Time.deltaTime;
        m_CollisionFlags = m_Ctl.Move(vMovemont);
        ...
        if (m_bJump)
        {
            ...
            {
                m_Animation.CrossFade(m_Jump.name, 0.0f);
            }
            else
            {
